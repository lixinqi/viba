...
- **内联要摊到底**：链绕回一个正在摊的定义时展不下去——`A = A * $x int` 是把 A 写成它自己，绕别名（`B = A`）或两个定义互指都算。这种类型是写错，判定与 `serialize` 都给 `VibaProgramErr`；地址层自己不抛（第 5.1 节那些格子不能抛），照原样把那一个成员交出来，要不要拒绝由查的人问 `VibaAccess.inline_cycle`（返回绕回的那个定义名，摊得开则返回 `nil`）。
...
| 接口 | 说明 |
|---|---|
| `VibaNode[Data]` | 一个节点：类型侧的一段描述符，加上实现侧的一份数据；其余格子都是第 5.4 节的访问器 |

...
- **实例**：实现方那边的数据，实际是什么——类型（type）在现实里的那份。它可以是 Python 对象、JSON、一段二进制。这里说的实例是那份实际数据，不是泛型实例化出来的类型体。

写工具、做校验、查问题的时候，都要求同一件事：**不预先知道类型长什么样，也能按类型把实例里的数据读出来**。具体就是给个地址——"`$len` 这一段"、"第 2 个元素"、"键 `k`"——问它在不在、把它取出来、走到里面接着问，直到取到叶子上的值。

麻烦的地方有四条：

- **实例长什么样由实现方定**。协议不规定语言和数据结构，也不要求实例长得像类型；两边对得上就行。
- **类型会改版**。实例可能是先前的类型做出来的；一份实例对着哪一版类型，是上层自己记的事，协议不管。
- **实现必须覆盖类型**。类型里列得出的每一条地址，实现那边都要走得通、落得到叶子；数据比类型多一点没关系（多出来的部分不在协议范围内）。反过来，类型里有而实现那边没有，就是不满足契约——问出来是 `false`，那是在说"没做到"。
- **读是有步骤的**。先问这一段有没有，再往里走一步，走到叶子才取值；容器还要能数、能枚举。

所以协议分成两半：**描述侧**把 `.viba` 源码编译成**描述符**（地图），只跟源码有关；**访问侧**按地图从实例里取值，由实现方交付。
...
每一步回来的还是同一个节点类型，所以能一直点；点不出来就抛异常，问有没有的那一类永不抛。`set` 没有下标，对它做 `at_index` 是 `VibaProgramErr`。

不想一步步点，就写一条路径交给便利函数：

- 一条路径走到底，直接读值（走不通给 `VibaProgramErr`）；
- 列出一个定义的所有字段，取到的收进表；
- 只问一句"这份实例是不是对着这版类型做的"，不建起点。
...
2. **地址**：一串步子（按 tag、按位置、按下标、按键）。每一步落在类型图的一段上，走之前先把那一段展开到可以寻址（第 4 节）。
3. **取值接口**：起点、问有没有、走一步、读叶子、数个数、取键，一共六个访问函数。实现方只交这六个；其余（一条路径走到底、列字段、链式写法）全由这六个拼出来，是糖（第 5 节）。

实例那边的两种对位、容器怎么数、契约是什么，第 9 节。
...
地址完全由类型侧的语言写成，不含实例那边的东西。

一步落在哪一段上，先要把那一段展开到可以寻址。展开只按类型里的定义走，不做别的猜测：

- **名字**：引用一个定义时，落到那个定义的体上。
- **泛型应用**：构造子指向一个泛型定义时，把实参代进形参，落到实例化的体上。实例那边写成同一个名字或同一个应用时，也按同一条规矩展开——类型里的成员与实例里的那一段因此一一对得上。
- **和 / 积 / 指数**：三种链结构相同，成员就是 `$elements`；链头是单位元就不算成员。
  类型里写了 `<<`（部分计算）的，先给掉再展开：读到的地址是给完之后的。指数的 `$elements[0]` 是结果，其余是参数，例如：`never <- $not_operand A` 的成员就是被禁止的那个操作数，`by_tag not_operand` / `get_not_operand()` 取到的就是它。
- **积的无标签成员是内联位**：一个不带标签的成员展开后是积（或一整条带标签的链）时，它的成员就摊在这里，摊进来的 tag 直接当这一层的成员——`A = $x int * $y int` 时，`B = A * $z int` 的成员就是 `$x` / `$y` / `$z`，没有多出来的一跳；摊完还是一样的规矩，可以一直摊下去。展开后是单位元（`Object` / `nil`，或落在它们上的名字）时它根本不是成员；其它不带标签的成员照旧按位置对位。同一个积里同一个 tag 出现两次是写错，判定与 `serialize` 都给 `VibaProgramErr`。
- **内联要摊到底**：链绕回一个正在摊的定义时展不下去——`A = A * $x int` 是把 A 写成它自己，绕别名（`B = A`）或两个定义互指都算。这种类型是写错，判定与 `serialize` 都给 `VibaProgramErr`；地址层自己不抛（第 5.1 节那些访问函数不能抛），照原样把那一个成员交出来，要不要拒绝由查的人问 `VibaAccess.inline_cycle`（返回绕回的那个定义名，摊得开则返回 `nil`）。
- **在这之前先查一遍**：上面两条（重标签、内联成环）是类型自己的事，跟实例无关，所以有一份一元检查（`viba/check_tag_and_inline.py`，函数同名）：它从每个定义出发走一遍，碰到的每个积都查，不受"这次拿它跟谁比"影响。判定与 `serialize` 里那两条 `VibaProgramErr` 是兜底，不是发现它的地方。
  ...
  CheckTagAndInline =
    ...
    <- Comments[{
        类型自己的一元检查：每个定义走过的每个积，摊开以后 tag 不得重复，内联链要摊到底。
        Ok(nil) 是没问题；$viba_program_err 说的是第一条错。
        $config 说哪些写下来的名字算单位元；accessor 在检查内部按它建，调用方不持有。
        它只读描述符，不碰实例，所以没有 [Data]。
      }]
  ```
- **无标签的和可以省层**：和的分支都没有标签、且至多一个内节点（叶子不算内节点）时，实例可以直接写那一支的内容，不必套上和这一层。这时 `[$x, 0]` 得到那一支的实例，`[$x, $a]`（标签落在那一支里）也穿得过去；叶子分支按值的种类对位——`int | str` 写 `7` 落在 `int` 上，`int | nil` 写 `nil` 落在 `nil` 上。两个以上内节点的和不能省（分不清是哪一支），带标签的和按 tag 寻址。
- **这几种写法没有成员**：字面量、代码块、单位元与零元，到此为止，成员为空。

展开只动类型那一侧，不动数据：数据里没有的地址照样答 `false`；反过来，实现里的数据也可以多过类型（第 9 节）。

## 5. 访问协议

本节的定义都属于访问侧：它们都带 `[Data]` 形参，`Data` 是"实例本身的类型"这个空位，实现方把自己的数据类型填进去。第 6–8 节那些不带 `[Data]` 的定义属于描述侧。**凡是带 `[Data]` 的定义，实现方必须一条不落都实现。**
...
| `VibaHas` | 这一步有没有。答 `false`：类型里没这个地址，或者实现那边没有这一段——后者是没覆盖类型 |
...
`Data` 是形参，实现方把它绑到自己那份数据表示上。数据全程不透明，只有 `VibaLeaf` 会把它翻成类型侧的五种字面量。

**起点只配成对、不做别的检查**：`$root` 把描述符和数据配成节点就返回。版本不归协议管：一份实例对着哪一版类型做的，由上层自己记、自己核。
...
- `VibaListFields`：按 `$definition` 展开后的成员（内联摊开、单位元不算，见第 4 节）逐个 `VibaGet`，取到的收进表；缺的字段不进表。
...
实现方交付一份 `VibaAccess[自己那份数据类型]`：六个访问函数齐全，才算把第 5.1 节的接口实现完整。节点上的访问器由框架补（第 5.4 节），不是实现方的事。
...
会抛的只有"取值"那一类：`by_tag` / `by_field_index` / `at_index` / `at_key` / `leaf` / `len` / `keys`。**问"有没有"和"试着取"的那一类不抛**，见下一段「落到 Python 对象上」。
...
便捷函数由框架按第 5.1 节的接口补上，不是实现方的额外义务：实现方只承诺 `VibaAccess` 那六个访问函数。
...
不带标签的成员没有名字，不进名字表，只能按位置查。内联摊进来的成员也没写在这个定义上，名字表里同样没有它——它仍挂在原本那个定义的名字底下（第 4 节）。
...
有一条约定在这里点出，它跟第 5.4 节直接相关：**形态查询只看写出来的链头**。成员写的是一个名字（`$xs Names`）时，形态识别不算；想刨到底，得顺着 `MemberResolvedDefinition` 一层层问。

## 9. 契约与约定

**契约**：实现必须覆盖类型。类型里列得出的每一条地址，在数据里都要走得通，并落得到叶子。

- 走得通时，`VibaGetByPath` 直接给出细节。
- 走不通时，`VibaHas` 答 `false`。这是"实现未覆盖类型"的事实记录。
- 实现中的数据可以多于类型。超出类型的部分不在本协议范围内：地址只能来自描述符，没有走到它的路径。

`VibaProgramErr` 只表示实现侧取不到（句柄失效、远端超时等），不表示缺席。

**约定**：

- **规范化（链式）**：反射读到的描述是规范化过的。主链压成一条链：`A | B | C` 与 `(A | B) | C` 是同一条描述，二元写法不出现。支链不压：`A | (B | C)` 是元素 `A` 加一条次级链，和 `A | B | C` 不一样，`$right` 侧的分组一直留着。
- **定义平铺**：定义都在文件这一层，全名即 `模块名.定义名`。
- **单位元不是成员**：和链 / 积链 / 指数链链头的 `Object` / `nil` / `Oneof` / `never` 是单位元，不进 `$members`；积里不带标签的单位元成员也一样不算。
- **两种对位**：带标签的成员按 tag 对位，tag 写出来是什么就记什么（含 `$`）；不带标签的按位置对位，没有名字。积里不带标签的成员是内联位（第 4 节）：摊进来的 tag 按 tag 对位，摊不进来的仍按位置对位。
- **内联进来的成员不算写在这里**：名字表只收这个定义自己写出来的成员；摊进来的 tag 仍挂在它原本所在的定义下（第 7 节）。
- **名字就是名字**：描述符里的引用不会在这里被替换；解析是另一条查询。
- **叶子五种**：bool / int / float / str / nil。

## 10. 实现要求

- 交付一份 `VibaAccess[自己那份数据类型]`，六个访问函数齐全。
- 容器要能数、能枚举：`$length` 给元素个数，`$keys` 给 `dict` 的键；枚举序列靠它们加 `$at_index`。
- 起点只给节点：`$root` 不做别的检查；版本由上层自己管。
- 节点上的便捷函数不用实现方提供，由框架按接口拼（第 5.4 节）。
- 只读：协议里不提供写操作。
- 不报名字、不自省、不上报 schema：实现只需在内部把描述符里的 tag 对到自己那边的键上。类型侧的结构一律以 `.viba` 源码为准。

...
```python
root = access(rule).root(witness)      # 起点：先核版本，对不上直接 Err
...
实现方交付一份 `VibaAccess[自己那份数据类型]`：七格函数齐全，才算把第 5.1 节的接口实现完整。`$root` 里那一次版本核对是强制的（第 5.1 节）。节点上的访问器由框架补（第 5.4 节），不是实现方的事。

`$file_hashes` 是数据侧对自己版本的声明：这份数据是按哪些设计文件、哪一版做出来的。它是只读的。`$root` 必须拿它核一次版本（第 5.1 节）；第 5.3 节的 `VibaVersionMatches` 是同一个核对的另一种用法，用于不建起点、先问一句的场合。

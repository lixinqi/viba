...
2. **地址**：一串步子（按 tag、按位置、按下标、按键）。每一步落在设计图的一段上，走之前先把那一段展开到可以寻址（第 4 节）。
...
一步落在哪一段上，先要把那一段展开到可以寻址。展开只按设计里的定义走，不做别的猜测：
...
展开只动设计那一侧，不动数据：数据里没有的地址照样答 `false`；反过来，实现里的数据也可以多过设计（第 9 节）。
...
**起点只配成对、不做别的检查**：`$root` 把描述符和数据配成节点就返回。版本不归协议管：一份材料对着哪一版设计做的，由上层自己记、自己核。

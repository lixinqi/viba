...
- **设计会改版**。材料得声明自己是按哪一版设计做的；版本对不上，说明材料依据的是已被改写或已删除的设计，继续读没有意义。
...
3. **取值接口**：起点、问有没有、走一步、读叶子、数个数、取键、报版本，一共七格。实现方只交这七格；其余（一条路径走到底、列字段、链式写法）全由这七格拼出来，是糖（第 5 节）。

材料那边的两种对位、容器怎么数、版本怎么核、契约是什么，第 9 节。
...
| `VibaFileHashes` | 这份数据是按哪些设计文件做出来的：那些文件的 `$file_hash` 集合。用于校验版本 |

`Data` 是形参，实现方把它绑到自己那份数据表示上。数据全程不透明，只有 `VibaLeaf` 会把它翻成设计侧的五种字面量。

**起点不核版本**：`$root` 只把 (描述符, 数据) 配成对。数据声明的哈希用 `$file_hashes` 问得到，要不要核、怎么核由上层定——第 5.3 节的 `VibaVersionMatches` 是现成的那一步。
...
实现方交付一份 `VibaAccess[自己那份数据类型]`：七格函数齐全，才算把第 5.1 节的接口实现完整。节点上的访问器由框架补（第 5.4 节），不是实现方的事。

`$file_hashes` 是数据侧对自己版本的声明：这份数据是按哪些设计文件、哪一版做出来的。它是只读的，给上层核版本用（第 5.3 节的 `VibaVersionMatches`）。
...
便捷函数由框架按第 5.1 节的接口补上，不是实现方的额外义务：实现方只承诺 `VibaAccess` 那七格。
...
- 起点不核版本：`$root` 只给节点；`$file_hashes` 照样要有，`VibaVersionMatches` 用它。

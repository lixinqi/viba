...
`DictLiteral[(a, 0)]`, in the order given. `viba.builder.code(text)` writes a
`{...}` block, and `viba.builder.add_import(vb, module, alias)` adds an import —
it stands outside the builder so that every `vb.<name>` is a definition.
...
__all__ = ["Builder", "tag", "code", "add_import"]
...
class _Name(_Expr):
    ...
    def __setitem__(self, key, body) -> None:
        ...
        definition = viba_ast.GenericDefinition(
            _definition_name(self.path), params, _ast(_wrap(body)))
...
class Builder:
    """One Viba file under construction: a header, then definitions in order.

    The header is whatever stands above the definitions (imports, comments)
    and is kept exactly as written; the definitions are what attribute and
    subscript assignment put in, rendered canonically by `str(vb)`.
    """

    def __init__(self, header: str = ""):
        object.__setattr__(self, "_header", header.strip())
    ...
    def __setattr__(self, name: str, value) -> None:
        ...
        if name.startswith("_"):
            ...
            return
        self._add(viba_ast.TypeDefinition(_definition_name(name), _ast(_wrap(value))))
    ...
    def __str__(self) -> str:
        """The whole file: the header as written, then the definitions."""
        ...
        definitions = viba_ast.unparse(viba_ast.Module(imports + rest))
        if not self._header:
            return definitions
        if not definitions:
            return self._header

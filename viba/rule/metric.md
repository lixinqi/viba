...
> **已废弃，不再维护。** 度量层与规则层到此为止：`viba/rule/` 不再改，也不再跟着
> 核心演进。取代它的是**可执行模块**——一次度量就是一次调用，写成会被跑起来的
> 模块（`viba-interpreter.md`），证据是那次运行里的材料，判定交给 `<<` 与类型检查。
>
> 下面保留原文，只作参考。
...
三条约束写在 `viba/rule/metric.viba` 里：
...
`MetricObject` 与 `Metric[CoreFunc]` 也在 `viba/rule/metric.viba` 里：
...
4. **证据**：`$call_instance` 底下是一次调用（第 3 节的 after call）——参数位是
   `Prepare`，结果位是量出来的值。

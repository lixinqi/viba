...
一条规则（Rule）是以 `Metric` 与字面量为叶子的 Viba 类型描述，声明材料必须满足的结构与断言；一份呈证材料（Witness）也是一个 Viba 类型。判定就是子类型判定 `witness <: rule`。规则层用到的 `RuleObject` 标记与 `Predicate` / `Metric` / `PredicationFailed` / `not` 都只是 Viba 类型写法。

Rule 里可以带谓词（Predicate）与禁止性字段（`not`），书写形式见第 4 节与第 7 节。

## 2. 规则标记

规则是积类型，链头带标记 `RuleObject`（定义见 `viba/rule/rule.viba`）。

- `RuleObject` 与 `Object` 完全同义：积类型单位元，基数为 1。
- 和类型不是规则：和单位元就是 `Oneof`，它只说"这是和类型"，不说"这是规则"。规则里要用和，就把它写成某个字段的类型。

标记只是名字层面的声明：判定语义上 `RuleObject` 就是 `Object`。它唯一的额外含义是：一个 module 中，只有带标记的定义才会被认为是规则；其余定义（Metric 名字、辅助类型）不参与规则枚举。
...
一条附带规则：不带标签的字段本身是积时，它是内联位——它的字段摊进这一层，不占位置（`viba-reflect.md` 第 4 节）。于是 `RuleObject * CommonFields * $assert_x ...` 里的 `CommonFields` 摊成它自己的字段，与逐个写出来同义；写成单位元（`Object` / `nil`）的不带标签字段则根本不是字段。
...
- **标记**：Rule 用 `RuleObject` 起头；Witness 自己不带标记，从 tag 起头（带了标记会被当成规则枚举）。

...
- **名字出不去**。块结束后 `x` 就不存在了；块的结果、以及块交给别人的东西（惰性实参的 getter）
  仍然看得见它。
- **材料里不能放绑定**。`$point (p := …  p)` 这种写法会被拒："a binding belongs in a value, not
  inside material"——材料是照着写下来的数据，里面不跑表达式。要给材料一个值，就让一个函数把它
  答出来。
...
- **可能会用不上的参数，用 `ParametersLazyEvaluated` 标起来。** 一个函数写成
  ...
  责任推给了每一个实现者。标记只改实参怎么给：判定层和描述符层读到的类型就是 `F`。
...
```
  ...
  **必须给全**，少一个当场报错；没有参数的模块也要写那一格 `<< ()`。模块体里 `args.a` 是那个
  实参。别用 `__args__` 装"可能有也可能没有"的东西——那些是分支（第 8 条），不是实参。
